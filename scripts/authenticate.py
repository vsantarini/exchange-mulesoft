...
def fetch_token(client_id, client_secret):
    try:
        ...
        print("[ERROR] Connection error - check network access to anypoint.mulesoft.com")
        sys.exit(1)

    except requests.exceptions.Timeout:
        print("[ERROR] Request timed out during authentication.")
        sys.exit(1)
...
def validate_token(data):
    ...
    for field in required:
        if field not in data:
            print(f"[ERROR] Token response missing field: {field}")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    print("[INFO] Authenticating to Anypoint Platform...")
