...
def create_contract(contract, app_id, org_id, env_id, token):
    ...
    print(f"[OK] Contract created: app={contract['appName']} -> asset={contract['assetId']}")
    return result
...
def update_contract(contract_id, contract, app_id, org_id, token):
    ...
    response.raise_for_status()
    print(f"[OK] Contract updated: id={contract_id} status={payload['status']}")
...
if __name__ == "__main__":
    ...
    for contract in contracts:
        ...
        if not app_id:
            print(f"[WARN] App ID not found for: {app_name}, skipping.")
            continue
